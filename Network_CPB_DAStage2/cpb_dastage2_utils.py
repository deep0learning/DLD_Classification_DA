...
def plotAccuracy(x, y, figName, lineName, savePath):
    ...
    plt.plot(x, y, linewidth=1.0, linestyle='-', label=lineName)
    plt.title('Accuracy')
...
def plotLoss(x, y1, y2, y3, y4, figName, line1Name, line2Name, line3Name, line4Name, savePath):
    plt.figure(figsize=(20.48, 10.24))
    plt.plot(x, y1, linewidth=1.0, linestyle='-', label=line1Name)
    plt.plot(x, y2, linewidth=1.0, color='red', linestyle='--', label=line2Name)
    plt.plot(x, y3, linewidth=1.0, color='orange', linestyle='-.', label=line3Name)
    plt.plot(x, y4, linewidth=1.0, color='pink', linestyle=':', label=line4Name)
    plt.title('Loss')
    plt.xlabel('Iteration', fontsize=16)
