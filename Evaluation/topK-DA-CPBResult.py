...
def getInformation(file):
    ...
    with open(file, 'r') as f:
        ...
        for l in range(len(lines)):
            ...
            if _line_information.startswith('epoch:'):
                ...
                _df_loss = float(splitted_info[3][splitted_info[3].find('[') + 1:splitted_info[3].rfind(']')])
                _cpb_loss = float(splitted_info[4][splitted_info[4].find('[') + 1:splitted_info[4].rfind(']')])

                epoch.append(_eps)
                training_accuracy.append(_training_accuracy)
                g_loss.append(_g_loss)
                df_loss.append(_df_loss)
                cpb_loss.append(_cpb_loss)
    ...
    for e, tr_acc, g_l, df_l, cpb_l, src_tst_acc, src_tst_fsc, tar_tst_acc, tar_tst_fsc in zip(epoch,
                                                                                               training_accuracy,
                                                                                               g_loss,
                                                                                               df_loss,
                                                                                               cpb_loss,
        ...
        summarized_list.append(
            [e, tr_acc, g_l, df_l, cpb_l, src_tst_acc, src_tst_fsc, tar_tst_acc, tar_tst_fsc])
...
def main():
    top_k = 20
    file_dir = 'D:/DLD_Classification_DomainAdaptation_Experimental_Results/Self-Comparison/self-compare/cpb/'
