...
import cpb_dastage1_utils as utils
import time


class cpb_dastage1_model(object):
    ...
    def build_model(self):
        ...
        self.cpb_features = self.contentPreservedBlock([self.target_stage0_features, self.target_stage1_features],
                                                       scope_name='stage1_cpb',
                                                       is_training=self.is_training)
        ...
        with tf.variable_scope('loss'):
            ...
            self.cpb_loss = tf.reduce_mean(tf.abs(self.target_stage0_features - self.cpb_features))

            self.g_loss = self.target_stage1_gloss + self.target_label_gloss + 10 * self.cpb_loss
        ...
        with tf.variable_scope('variables'):
            ...
            self.stage1_cpb_var = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='stage1_cpb')
            ...
            source_stage1_var = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='stage1_source')
            target_stage1_var = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='stage1_target')
            for sv in source_stage1_var:
                modified_name = sv.name.replace('_source', '')
                self.reload_var_unshared_source[modified_name[:-2]] = sv
            for tv in target_stage1_var:
                modified_name = tv.name.replace('_target', '')
                self.reload_var_unshared_target[modified_name[:-2]] = tv

        with tf.variable_scope('optimize'):
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope='stage1_target')):
                self.encoder_trainOp = tf.train.AdamOptimizer(self.lr, beta1=0.5).minimize(self.g_loss,
                                                                                           var_list=self.stage1_target_var)
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope='stage1_discriminator')):
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope='stage1_cpb')):
                self.cpb_trainOp = tf.train.AdamOptimizer(self.lr).minimize(self.cpb_loss, var_list=self.stage1_cpb_var)
        ...
        for e in range(1, self.eps + 1):
            for itr in range(self.itr_epoch):
                for m in range(2):
                    ...
                    _, _, _ = self.sess.run(
                        [self.discriminator_feature_trainOp, self.discriminator_label_trainOp, self.cpb_trainOp],
